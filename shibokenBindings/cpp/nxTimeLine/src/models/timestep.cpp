...
{
public:
    Private(TimeStep *owner,RulerContext *ctx, qint64 offset, qint64 unit, EdgeType lineType):
        owner(owner),
        offset(offset),
        unit(unit),
        lineType(lineType),
...
void TimeStep::Private::updateSubItems()
{
    if(subItems.empty() && lineType != EdgeType::UNDEFINED)
    {
        ...
        for(int i=0;i<subCount;i++){
            EdgeType subLineType = EdgeType::UNDEFINED;
            if(lineType < EdgeType::SMALLEST){
                subLineType = (EdgeType)((int)lineType+1);
            }
...
TimeStep::TimeStep(RulerContext* context, qint64 offset, qint64 unit, EdgeType lineType):
    QObject{context},
...
void TimeStep::onContextChanged()
{
    ...
    if(unit() == context()->highestUnit())
        ...
        setLineType((int)EdgeType::NORMAL);
    else if(unit() == context()->smallUnit())
        setLineType((int)EdgeType::SMALL);
    else if(unit() == context()->smallestUnit())
        setLineType((int)EdgeType::SMALLEST);
    else
...
{
    return (value()%d->context->normalUnit() == 0 && d->lineType != EdgeType::NORMAL) || (d->context->stopValueByUnit(unit()) == value());
}
