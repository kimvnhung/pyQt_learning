...
QAVVideoFrame QAVVideoFrame::convertTo(AVPixelFormat fmt) const
{
    ...
    // qDebug()<<__LINE__<<__FUNCTION__<<size();

    // Adjust the destination size to 800x600.
    int destinationWidth = 800;
    int destinationHeight = 600;
    ...
    if (ret == -1) {
        qWarning() << __FUNCTION__ << "Colorspace not supported";
        sws_freeContext(ctx);
        return QAVVideoFrame();
    }

    // Use the adjusted destination size for the result frame.
